...
def _read_tunnel_url_file():
    """Live Cloudflare tunnel URL written by denuvo-tunnel (never a raw IP)."""
    from urllib.parse import urlparse
    candidates = []
    env_file = os.environ.get("PUBLIC_URL_FILE", "").strip()
    if env_file:
        candidates.append(Path(env_file))
    candidates.append(PROJECT_ROOT.parent / "denuvo-data" / "public-url.txt")
...
def _public_base_url():
    ...
    Prefers the live tunnel file, then PUBLIC_URL env, then RAILWAY_PUBLIC_DOMAIN.
    Never returns a raw IP — use Cloudflare tunnel URL instead."""
    tunnel = _read_tunnel_url_file()
    if tunnel:
        return tunnel
    ...
    if explicit:
        ...
        try:
            from urllib.parse import urlparse
            host = urlparse(explicit).hostname or ""
            if _is_ip_host(host):
                explicit = ""
        except Exception:
            explicit = ""
    ...
    if render:
        if not re.match(r"^https?://", render, re.IGNORECASE):
            render = "https://" + render
        return render
...
def build_thin_zip(out, app_id, game_name, generation_mode, token_b64, steam_id, fake_mode, base_url):
    ...
    Layout:
        ...
        opensteam-modes.txt              ← "primary=<mode>"
    ...
    v1_ini_content = (
        "# Generated by OpenSteam bot. ColdClientLoader v1 (Goldberg/Rat431).\n"
        "[SteamClient]\n"
    ...
    (out / "opensteam-modes.txt").write_text(f"primary={primary}\n", encoding="utf-8")
    ...
    try:
        meta = {
            ...
            # under _OPENSTEAM_NEW_* keys before calling us. Empty strings
            # mean "no data" (skip the cache update on the Node side).
            "session_source": os.environ.get("_OPENSTEAM_SESSION_SOURCE") or None,
            "steam_login":    os.environ.get("_OPENSTEAM_NEW_STEAM_LOGIN") or None,
            "steam_password": os.environ.get("_OPENSTEAM_NEW_STEAM_PASSWORD") or None,
            "steam_id":       os.environ.get("_OPENSTEAM_NEW_STEAM_ID") or None,
            "refresh_token":  os.environ.get("_OPENSTEAM_NEW_REFRESH_TOKEN") or None,
            "guarded":        (os.environ.get("_OPENSTEAM_NEW_STEAM_GUARDED") or "true").lower() == "true",
            ...
            "steampass_token": os.environ.get("_OPENSTEAM_NEW_STEAMPASS_TOKEN") or None,
            "steampass_calls": json.loads(os.environ.get("_OPENSTEAM_STEAMPASS_CALLS") or "[]"),
            "guard_code": os.environ.get("_OPENSTEAM_GUARD_CODE") or None,
            "guard_valid_until": os.environ.get("_OPENSTEAM_GUARD_VALID_UNTIL") or None,
        }
...
def build_multi_mode_zip(out, app_id, game_name, generation_mode, token_b64, steam_id, fake_mode):
    ...
    log("Built GBE payload (opensteam-modes/gbe/)")
    ...
    if cc_src.exists():
        ...
        log("Built V1 payload (opensteam-modes/v1/)")
    else:
...
def _new_auth_login(client, username, password, guard_code):
    ...
    resp = requests.post(begin_url, data={
        "device_friendly_name": "OpenSteam Bot",
...
def inject_ticket(cfg_path, token_b64, steam_id):
    ...
    for line in lines:
        ...
        if s.startswith("ticket="):
            ...
            new.append("account_name=OpenSteam")
            found_a = True
    ...
    if not (found_t and found_a and found_s):
        ...
        for line in new:
            ...
            if not injected and line.strip() == "[user::general]":
                if not found_a:
                    final.append("account_name=OpenSteam")
                if not found_s:
        ...
        if not injected:
            final += [
                "[user::general]", "account_name=OpenSteam",
...
def main(app_id, game_name, steampass_uuid, generation_mode="gbe"):
    ...
    if fake_mode:
        ...
        # ── STEAMAUTH MODE (OpenSteam Auth Service guard codes) ──
        ...
        # ── Cached session env (read early — refresh-only mode may have no steampass password) ──
        ...
        refresh_only = steampass_disabled and bool(cached_login and cached_refresh_token)
        ...
        if session_source is None and (not sp_login or not sp_password) and not refresh_only:
            log("ERROR: STEAMPASS_LOGIN and STEAMPASS_PASSWORD env vars required")
            sys.exit(1)
        if session_source is None and refresh_only and (not sp_login or not sp_password):
            log("Refresh-token-only mode — steampass credentials not required")
            sp_login = sp_login or cached_login
        ...
        os.environ["_OPENSTEAM_SESSION_SOURCE"] = session_source or ""
        os.environ["_OPENSTEAM_NEW_REFRESH_TOKEN"] = new_refresh_token or ""
        os.environ["_OPENSTEAM_NEW_STEAM_LOGIN"] = steam_login or ""
        os.environ["_OPENSTEAM_NEW_STEAM_PASSWORD"] = steam_password or ""
        os.environ["_OPENSTEAM_NEW_STEAM_ID"] = steam_id or ""
        os.environ["_OPENSTEAM_NEW_STEAM_GUARDED"] = "true" if guarded else "false"
    ...
    if base_url:
        ...
        self_hosted = os.environ.get("SELF_HOSTED_DOWNLOADS", "").strip().lower()
        if self_hosted not in ("", "false", "0", "no", "off"):
            log(
                "ERROR: No Cloudflare tunnel URL — refusing 50 MB embedded fallback. "
                "Start denuvo-tunnel and ensure denuvo-data/public-url.txt exists."
            )
            sys.exit(1)
